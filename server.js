...
const stores = {
  ...
  si: { name: 'Slovenia', flag: '🇸🇮', url: 'https://noriks.com/si', ck: 'ck_15d449f9a27f0447ba3d8d41ce81f3b75e52337e', cs: 'cs_4f5cf5ae37b447512b7d23a50d8ba2d0f268ed6b' },
...
const storeCurrencies = { hr: 'EUR', cz: 'CZK', pl: 'PLN', sk: 'EUR', hu: 'HUF', gr: 'EUR', it: 'EUR', si: 'EUR' };
...
async function warmRAM() {
  ...
  try {
    ...
    // Warm callcenter-orders cache (background, don't block startup)
    setTimeout(() => {
...
async function fetchOneTimeBuyers(storeFilter = null, forceFull = false) {
  ...
  const snapshotFile = path.join(DATA_DIR, 'buyers-full-snapshot.json');
  let snapshot = readJson(snapshotFile, null);
  const now = Date.now();
  const sixHours = 6 * 3600 * 1000;
  const needFullFetch = forceFull || !snapshot || !snapshot.timestamp || (now - snapshot.timestamp > sixHours);

  let allOrdersFlat = [];

  if (needFullFetch) {
    // Full fetch: get all orders from all stores
    console.log('[Buyers] Full fetch (snapshot rebuild)...');
      ...
      for (let page = 1; page <= 999; page++) {
        try {
          ...
          for (const o of orders) {
            allOrdersFlat.push({ storeCode, email: (o.billing?.email || '').toLowerCase(), orderId: o.id, total: o.total, currency: o.currency, dateCreated: o.date_created, billing: o.billing, lineItems: o.line_items, status: o.status });
          }
          if (orders.length < 100) break;
        } catch { break; }
      }
    }
    snapshot = { timestamp: now, allOrders: allOrdersFlat };
    writeJson(snapshotFile, snapshot);
    console.log('[Buyers] Snapshot saved:', allOrdersFlat.length, 'orders');
  } else {
    // Incremental: fetch only orders after snapshot timestamp
    console.log('[Buyers] Incremental fetch (since ' + new Date(snapshot.timestamp).toISOString() + ')...');
    const afterDate = new Date(snapshot.timestamp).toISOString();
    const newOrders = [];
    for (const [storeCode, config] of Object.entries(storesToFetch)) {
      if (!config) continue;
      try {
        const orders = await wcApiRequest(storeCode, 'orders', {
          per_page: 100, status: 'processing,completed', orderby: 'date', order: 'desc', after: afterDate
        });
        if (Array.isArray(orders)) {
          for (const o of orders) {
            newOrders.push({ storeCode, email: (o.billing?.email || '').toLowerCase(), orderId: o.id, total: o.total, currency: o.currency, dateCreated: o.date_created, billing: o.billing, lineItems: o.line_items, status: o.status });
          }
        }
      } catch { /* skip */ }
    }
    // Merge: add new orders, deduplicate by storeCode+orderId
    const existingKey = new Set(snapshot.allOrders.map(o => o.storeCode + '_' + o.orderId));
    for (const o of newOrders) {
      if (!existingKey.has(o.storeCode + '_' + o.orderId)) {
        snapshot.allOrders.push(o);
      }
    }
    snapshot.timestamp = now;
    writeJson(snapshotFile, snapshot);
    allOrdersFlat = snapshot.allOrders;
    console.log('[Buyers] Incremental: ' + newOrders.length + ' new, ' + allOrdersFlat.length + ' total');
  }

  // Derive one-time buyers from allOrdersFlat
  for (const [storeCode, config] of Object.entries(storesToFetch)) {
    if (!config) continue;
    const storeOrders = allOrdersFlat.filter(o => o.storeCode === storeCode);
    ...
    for (const [email, data] of Object.entries(customerOrders)) {
      ...
      const orderItems = (order.lineItems || order.line_items || []).map(extractLineItemDetails);
      ...
      allBuyers.push({
        ...
        registeredAt: order.dateCreated || order.date_created || '', orderStatus: order.status || '',
        callStatus: savedData.callStatus || 'not_called', notes: savedData.notes || '',
...
async function fetchPendingOrders(forceFull = false) {
  ...
  // Incremental: use modified_after if we have a recent full fetch
  const pendingStateFile = path.join(DATA_DIR, 'pending-last-fetch.json');
  const pendingState = readJson(pendingStateFile, null);
  const now = Date.now();
  const useModifiedAfter = !forceFull && pendingState && pendingState.timestamp && (now - pendingState.timestamp < 15 * 60 * 1000);
  ...
  // If incremental, merge with previous data
  if (useModifiedAfter && pendingState && pendingState.lastOrders) {
    const newIds = new Set(allOrders.map(o => o.id));
    for (const prev of pendingState.lastOrders) {
      if (!newIds.has(prev.id)) allOrders.push(prev);
    }
  }
  ...
  // Save state for incremental fetching
  if (!useModifiedAfter) {
    writeJson(pendingStateFile, { timestamp: now, lastOrders: allOrders });
  } else {
    writeJson(pendingStateFile, { timestamp: pendingState.timestamp, lastOrders: allOrders });
  }
...
async function buildPaketomatiCache(forceFull = false) {
  ...
  // --- Incremental logic with known IDs ---
  const knownIdsFile = path.join(DATA_DIR, 'paketomati-known-ids.json');
  const knownState = readJson(knownIdsFile, null);
  const twoHours = 2 * 3600 * 1000;
  const needFullScan = forceFull || !knownState || !knownState.timestamp || (Date.now() - knownState.timestamp > twoHours);
  const knownMkIds = new Set(knownState?.knownMkIds || []);
  ...
  while (emptyCount < 3) {
    try {
      ...
      emptyCount = 0;
      let allKnownInBatch = !needFullScan;
      ...
      for (const o of orders) {
        ...
        if (!needFullScan && o.mk_id && !knownMkIds.has(String(o.mk_id))) {
          allKnownInBatch = false; // found a new order
        }
      }
      ...
      // Incremental: stop scanning once we hit a batch where all shipped noriks orders are known
      // But still check recent orders (last 7 days) for delivery event updates
      if (!needFullScan && allKnownInBatch) {
        const oldestInBatch = lastDate || '';
        if (oldestInBatch && oldestInBatch < sevenDaysAgo) {
          hitKnown = true;
          break;
        }
      }
  ...
  console.log('[Paketomati]', needFullScan ? 'Full scan:' : 'Incremental scan:', 'scanned', offset, 'orders, found', allShippedOrders.length, 'shipped noriks this year');
  const statusData = loadPaketomatStatus();
  const paketomatOrders = [];

  // Process in batches — skip get_document for known orders (unless recent, for delivery event updates)
  for (let i = 0; i < allShippedOrders.length; i += 20) {
    ...
    const docPromises = batch.map(async (order) => {
      ...
      if (!mkId) return null;
      // Skip API call for known orders older than 7 days (no delivery event changes expected)
      if (!needFullScan && knownMkIds.has(String(mkId))) {
        const shipDate = order.shipped_date || order.doc_date || '';
        if (shipDate && shipDate < sevenDaysAgo) return null;
      }
      try {
  ...
  // If incremental and we have previous cache, merge with previous orders not in current scan
  if (!needFullScan) {
    const prevCache = readJson(path.join(DATA_DIR, 'paketomati-cache.json'), null);
    if (prevCache && Array.isArray(prevCache.orders)) {
      const currentIds = new Set(paketomatOrders.map(o => o.id));
      for (const prev of prevCache.orders) {
        if (!currentIds.has(prev.id)) paketomatOrders.push(prev);
      }
    }
  }
  ...
  // Save known IDs for incremental scanning
  const allKnownIds = allShippedOrders.map(o => String(o.mk_id)).filter(Boolean);
  // Merge with previous known IDs on incremental
  if (!needFullScan) { for (const id of knownMkIds) allKnownIds.push(id); }
  writeJson(knownIdsFile, { timestamp: Date.now(), knownMkIds: [...new Set(allKnownIds)] });
...
// ========== STARTUP CACHE WARMING + BACKGROUND REFRESH ==========
async function warmAllCaches() {
  console.log('[Cache] Warming all caches...');
  const start = Date.now();
  try {
    const results = await Promise.allSettled([
      fetchAbandonedCarts(),
      fetchPendingOrders(),
      buildPaketomatiCache()
    ]);
    const labels = ['AbandonedCarts', 'PendingOrders', 'Paketomati'];
    results.forEach((r, i) => {
      if (r.status === 'fulfilled') console.log('[Cache] ✓ ' + labels[i] + ' warmed');
      else console.error('[Cache] ✗ ' + labels[i] + ' failed:', r.reason?.message || r.reason);
    });
    console.log('[Cache] All caches warmed in ' + ((Date.now() - start) / 1000).toFixed(1) + 's');
  } catch (e) {
    console.error('[Cache] Warming error:', e.message);
  }
}

// Background refresh every 5 minutes (optimized: incremental fetching)
let _bgRefreshCounter = 0;
function startBackgroundRefresh() {
  setInterval(async () => {
    ...
    const results = await Promise.allSettled([
      // 1. Abandoned carts (uses its own 300s cache internally)
      fetchAbandonedCarts().then(async carts => {
      ...
      // 2. Pending orders (incremental with modified_after)
      fetchPendingOrders(forceFullPending).then(orders => {
        if (orders && orders.length > 0) dbWrite('pending', orders);
        console.log('[Cron] ✓ Pending:', (orders || []).length);
      }),
      // 3. One-time buyers (incremental with snapshot)
      fetchOneTimeBuyers(null, forceFullBuyers).then(buyers => {
        if (buyers && buyers.length > 0) {
          dbWrite('buyers', buyers);
          const buyersCacheFile = path.join(DATA_DIR, 'buyers-cache.json');
          writeJson(buyersCacheFile, { generated_at: Math.floor(Date.now()/1000), count: buyers.length, buyers });
      ...
      // 4. Paketomati (incremental with known IDs)
      buildPaketomatiCache(forceFullPaketomati).then(cache => {
  ...
  try {
    ...
    if (req.query.countries) {
      ...
      if (!countries.includes('all')) {
        orders = orders.filter(o => countries.includes(o.storeCode));
      }
    }
...
app.get('/api/statistics', (req, res) => {
  try {
    ...
    // Build set of cart IDs that belong to this period
    const periodCartIds = new Set();
    for (const c of carts) {
      const day = (c.abandonedAt || '').slice(0,10);
      if (day >= fromDate && day <= toDate) periodCartIds.add(c.id);
    }

    // 1. Called per day per country (abandoned cart IDs only), split current vs prev period
    const abCalledByDC = {};
    const abCalledPrevByDC = {};
    for (const [id, data] of Object.entries(callData)) {
      ...
      if (periodCartIds.has(id)) {
        abCalledByDC[key] = (abCalledByDC[key] || 0) + 1;
      } else {
        abCalledPrevByDC[key] = (abCalledPrevByDC[key] || 0) + 1;
      }
    }
    ...
    for (let d = new Date(end); d >= start; d.setDate(d.getDate()-1)) {
      ...
      const row = { date: ds };
      let tCalled = 0, tCalledPrev = 0, tNew = 0;
      for (const c of countries) {
        const called = abCalledByDC[ds+'_'+c] || 0;
        const newC = newCartsByDC[ds+'_'+c] || 0;
        const calledPrev = abCalledPrevByDC[ds+'_'+c] || 0;
        row[c] = {
          called,
          calledPrev,
          newCarts: newC,
        ...
        tCalled += called;
        tCalledPrev += calledPrev;
        tNew += newC;
      }
      row.total = { called: tCalled, calledPrev: tCalledPrev, newCarts: tNew };
      days = abDays;
      abDays.push(row);
    }
    ...
    let sumCalled = 0, sumCalledPrev = 0, sumNew = 0, sumTotal = 0, sumMCalled = 0, sumMNotCalled = 0;
    for (const c of countries) {
      sumTotal += monthTotalByCountry[c];
      sumMCalled += monthCalledByCountry[c];
      sumMNotCalled += monthNotCalledByCountry[c];
    }
    for (const row of abDays) { sumCalled += row.total.called; sumCalledPrev += row.total.calledPrev; sumNew += row.total.newCarts; }
    ...
    res.json({
      ...
      abMonthSummary: {
        countries: countries.reduce((acc, c) => {
          ...
          let cCalled = 0, cCalledPrev = 0, cNew = 0;
          for (const row of abDays) {
            cCalled += (row[c]?.called || 0);
            cCalledPrev += (row[c]?.calledPrev || 0);
            cNew += (row[c]?.newCarts || 0);
          }
          acc[c].totalCalled = cCalled;
          acc[c].totalCalledPrev = cCalledPrev;
          acc[c].totalNew = cNew;
          return acc;
        }, {}),
        totals: { called: sumCalled, calledPrev: sumCalledPrev, newCarts: sumNew, monthTotal: sumTotal, monthCalled: sumMCalled, monthNotCalled: sumMNotCalled }
      },
